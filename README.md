<p align="center">
  <strong>S. Mostafa</strong><br>
  Backend & Automation Developer | Python & .NET<br>
  📍 Iran | 🔗 <a href="https://github.com/SMostafaM">GitHub</a>
</p>

<p align="center">
  Backend and Automation Developer with strong experience in backend API development, web scraping, and browser automation using Python and .NET technologies. Skilled in building scalable APIs with FastAPI, Flask, and ASP.NET Core, as well as automation and data extraction solutions using Selenium, Playwright, Requests, and BeautifulSoup. Experienced with MongoDB and familiar with SQL Server and PostgreSQL databases. Also worked with Django-based projects and AI-assisted development workflows to improve code quality, refactoring, and productivity.
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/FastAPI-009688?style=for-the-badge&logo=fastapi&logoColor=white" />
  <img src="https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white" />
  <img src="https://img.shields.io/badge/.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white" />
  <img src="https://img.shields.io/badge/ASP.NET%20Core-512BD4?style=for-the-badge&logo=dotnet&logoColor=white" />
  <img src="https://img.shields.io/badge/Selenium-43B02A?style=for-the-badge&logo=selenium&logoColor=white" />
  <img src="https://img.shields.io/badge/Playwright-000000?style=for-the-badge&logo=playwright&logoColor=white" />
  <img src="https://img.shields.io/badge/Requests-00599C?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/BeautifulSoup-4B8BBE?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white" />
  <img src="https://img.shields.io/badge/SQL%20Server-CC2927?style=for-the-badge&logo=microsoftsqlserver&logoColor=white" />
  <img src="https://img.shields.io/badge/PostgreSQL-336791?style=for-the-badge&logo=postgresql&logoColor=white" />
  <img src="https://img.shields.io/badge/Django-092E20?style=for-the-badge&logo=django&logoColor=white" />
  <img src="https://img.shields.io/badge/HTML-E34F26?style=for-the-badge&logo=html5&logoColor=white" />
  <img src="https://img.shields.io/badge/CSS-1572B6?style=for-the-badge&logo=css3&logoColor=white" />
  <img src="https://img.shields.io/badge/AI-Assisted%20Development-000000?style=for-the-badge&logo=openai&logoColor=white" />
  <img src="https://img.shields.io/badge/Machine%20Learning-F7931E?style=for-the-badge&logo=tensorflow&logoColor=white" />
  <img src="https://img.shields.io/badge/Web%20Scraping-FF6F00?style=for-the-badge&logo=googlechrome&logoColor=white" />
  <img src="https://img.shields.io/badge/REST%20API-61DAFB?style=for-the-badge&logo=fastapi&logoColor=black" />
</p>

<p align="center">
  <strong>Core Strengths:</strong><br>
  Backend API Development | FastAPI & Flask Development | ASP.NET Core APIs | Web Scraping & Browser Automation | Automation Tools & Bots | MongoDB Integration | Data Collection & Processing | API Integration | Code Refactoring & Optimization | AI-Assisted Development
</p>

<p align="center">
  <strong>Sample Projects:</strong><br>
  <em>Automation & Scraping Services:</em> Built automation workflows and scraping systems using Selenium, Playwright, Requests, and BeautifulSoup with API integrations and MongoDB storage.<br>
  <em>Python Backend APIs:</em> Developed RESTful APIs and backend services using FastAPI and Flask for automation and data-driven applications.<br>
  <em>.NET Backend Services:</em> Built backend APIs and database-driven services using ASP.NET Core and SQL Server.<br>
  <em>Django & PostgreSQL Experience:</em> Worked on backend CRUD and database integration tasks using Django and PostgreSQL.<br>
  <em>AI-Assisted Development:</em> Leveraged AI tools for debugging, optimization, refactoring, and improving development workflows.
</p>

<p align="center">
  <strong>Suitable Roles:</strong><br>
  Python Backend Developer | .NET Backend Developer | Backend API Developer | Automation Engineer | Web Scraping Engineer | Data Collection Developer
</p>